...
[NOTE]
If you are new to Testcontainers, please read https://testcontainers.com/guides/introducing-testcontainers[What is Testcontainers, and why should you use it?] to learn more about it.
...
We have added *Npgsql*, a Postgres ADO.NET Data Provider for talking to the Postgres database, and *xUnit.net* for testing our service.
...
Create a `DbConnectionProvider` class to hold ADO.NET connection parameters and create a method to get a database connection, as follows:
...
* We are calling `_dbConnectionProvider.GetConnection()` to get a database connection using ADO.NET.
* We have the `CreateCustomersTable()` method that creates the `customers` table if it does not already exist.
* We have the `Create(Customer)` method that inserts a new customer record into the database.
* We have the `GetCustomers()` method that fetches all rows from the `customers` table, populates data into `Customer` objects, and returns a list of `Customer` objects.
...
As we are using a Postgres database for our application, we added the Testcontainers Postgres https://www.nuget.org/packages/Testcontainers.PostgreSql[module] as a test dependency.
...
* We declare `PostgreSqlContainer` by passing the Docker image name postgres:15-alpine to the Postgres builder.
...
* We have a `ShouldReturnTwoCustomers()` test where we initialize `CustomerService`, insert two customer records into the database, fetch all the existing customers, and assert the number of customers.
* Finally, we dispose of the Postgres container in the `DisposeAsync()` member, which gets executed after all the test methods in that class are executed.

If you run the customer service test, you can see in the IDE's debug output that Testcontainers pulled the Postgres Docker image from DockerHub if it's not already available locally, started the container, and executed the test.
...
In addition to Postgres, Testcontainers provides dedicated https://www.nuget.org/profiles/Testcontainers[modules] for many commonly used SQL databases, NoSQL databases, messaging queues, etc. You can use Testcontainers to run any containerized dependency for your tests!

You can explore more about Testcontainers at https://www.testcontainers.com/.

== Further Reading

* https://testcontainers.com/guides/testing-an-aspnet-core-web-app/[Testing an ASP.NET Core web app]
